...
func TestGetNumberOfKeys(t *testing.T) {
	t.Run("Returns 2 as the number of keys", func(t *testing.T) {
		var numOfKeys uint16 = 2
		node := make(BNode, BTREE_PAGE_SIZE)
		binary.LittleEndian.PutUint16(node[2:4], numOfKeys)
		if got := node.getNumOfKeys(); got != 2 {
			t.Errorf("getNodeType() = %v, want %v", got, numOfKeys)
		}
	})
}
