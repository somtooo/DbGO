	...
	case BNODE_INTERNAL:
		...
		if position != 0 {
			...
			if position > 0 {
				...
				nodeAppendKV(new, idx, tree.new(mergedNode), mergedNode.getKey(0), nil)
			...
			if position < 0 {
				...
				nodeAppendKV(new, idx-1, tree.new(mergedNode), mergedNode.getKey(0), nil)
...
func shouldMerge(tree *BTree, node BNode, idx uint16, updated BNode) (int, BNode) {
	if updated.nbytes() > uint16(BTREE_PAGE_SIZE)/4 {
		return 0, BNode{}
	}
